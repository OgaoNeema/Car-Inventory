...
def draw_suv(canvas, x, y, scale, base_color, window_color, wheel_color, rim_color, secondary_color):
    ...
    canvas.create_rectangle(body_start, body_top, body_end, body_bottom,
                            fill=base_color, outline="black", width=3)

    # Roof and window frame
    canvas.create_polygon(x+30*scale, y-10*scale, x+100*scale, y-60*scale,
                          x+190*scale, y-60*scale, x+260*scale, y-10*scale,
                          fill=secondary_color, outline="black", width=3)

    # Windows
    canvas.create_polygon(x+45*scale, y-10*scale, x+100*scale, y-53*scale,
                          x+140*scale, y-53*scale, x+140*scale, y-10*scale,
                          fill=window_color, outline="black", width=2)
    canvas.create_polygon(x+145*scale, y-10*scale, x+145*scale, y-53*scale,
                          x+190*scale, y-53*scale, x+250*scale, y-10*scale,
                          fill=window_color, outline="black", width=2)

    # Wheels and rims
    for dx in [40, 190]:
        canvas.create_oval(x+dx*scale, y+60*scale, x+(dx+50)*scale, y+110*scale,
                           fill=wheel_color, outline="black", width=3)
        canvas.create_oval(x+(dx+10)*scale, y+70*scale, x+(dx+40)*scale, y+100*scale,
